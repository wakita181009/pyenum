...
- Q: What happens when a Rust variant name collides with a Python keyword or an enum-reserved dunder (e.g. `class`, `_value_`, `_missing_`, `__init__`)? → A: Compile-time rejection. The derive carries a known-forbidden set — Python keywords plus `enum`-module reserved names (`_name_`, `_value_`, `_missing_`, `_generate_next_value_`, `_ignore_`, `_order_`) plus dunders that `enum.EnumType` interprets specially (`__init__`, `__new__`, `__class__`, `__members__`, `__init_subclass__`, `__set_name__`) — and emits a compile error naming the offending variant. No auto-rename, no runtime deferral, no opt-out in v1.
...
- **FR-006**: The library MUST generate PyO3 conversion implementations (input extraction and output conversion compatible with PyO3 0.28's conversion traits) so that the Rust enum can appear directly in `#[pyfunction]`, `#[pymethods]`, and `#[pyclass]` field signatures without manual conversion code.
...
- The library targets PyO3 0.28 as the baseline; behavior on earlier or later PyO3 versions is out of scope for v1.
...
- Per-interpreter caching relies on PyO3 0.28's interpreter-state primitives (e.g., `GILOnceCell` or its 0.28 equivalent); behavior under the free-threaded (`--disable-gil`) Python build is not guaranteed in v1 and must be documented.
